...
public final class FlyWingLayer extends RenderLayer<FlyRenderState, FlyModel> {
    ...
    public void submit(PoseStack poseStack, SubmitNodeCollector collector, int packedLight, FlyRenderState state, float yRot, float xRot) {
        ...
        collector.submitModel(model, state, poseStack, RenderType.entityTranslucent(texture), packedLight, 0, state.outlineColor, null);
        ...
        if (blur > 0.4F) {
            ...
            for (float dz : GHOST_OFFSETS) {
                ...
                collector.submitModel(model, state, poseStack, RenderType.entityTranslucent(texture), packedLight, 0, 0x50FFFFFF, null);
            }
